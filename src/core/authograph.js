...
const remapFieldType = (schemaTypeMap, typeMap, type, newTypeList) => {
  if(type.ofType){
    const recurse = remapFieldType(schemaTypeMap, typeMap, type.ofType, newTypeList);
    if(recurse) {
...
export default class Authograph {
  ...
  bounder(resolve, bounds) {
    return (root, args, context) => {
      let permittedRoles = context.schema._permittedRoles||[];
      let oob = context.schema._oob;
      ...
      .forEach(arg => {
        ...
        .forEach(role => {
          ...
          let pass = Object.keys(bounds[arg][role])
          .every(bound => {
            if(!this.Bounds[bound]) {
              console.warn(new Error(`Invalid permission bounder ${bound}`));
              return true;
            }
            return this.Bounds[bound](arg, bounds[arg][role][bound], root, args, context);
          })
          if(!pass) {
            console.log(context);
            permittedRoles.splice(permittedRoles.indexOf(role),1);
            oob.push([context.parentType.name, context.fieldName, bounds, args]);
          }
        })
      })
  ...
  filterSchema(schema, pSet) {
    let permitTypes = _.intersection(Object.keys(pSet),Object.keys(schema._typeMap));
    let newTypeList = GraphQLTypes.concat(permitTypes);
    let filteredSchema = Object.create(schema);
    ...
    .reduce((typeMap, typeKey) => {
      ...
      typeOverlay._fields = _.intersection(Object.keys(pSet[typeKey]),Object.keys(typeOverlay._fields))
      .reduce((fieldMap,fieldKey) => {
        ...
        if(fieldOverlay.type) {
          ...
          .filter(arg => Object.keys(pSet[typeKey][fieldKey]).indexOf(arg.name) !== -1 || arg.name === "_");

          fieldOverlay.resolve = this.bounder(fieldOverlay.resolve,pSet[typeKey][fieldKey])
    ...
    filteredSchema._queryType = filteredSchema._typeMap[filteredSchema._queryType.name];

    if(filteredSchema._mutationType && filteredSchema._typeMap[filteredSchema._mutationType.name]) {
      filteredSchema._mutationType = filteredSchema._typeMap[filteredSchema._mutationType.name];
    } else {
  ...
  middleware(schema) {
    return (req, res, next) => {
      ...
      .then(pSet => {
        return this.filterSchema(schema, pSet);
      })
      .then(schema => {
        if(schema) {
